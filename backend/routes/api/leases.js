...
const express = require('express'); // const { check } = require('express-validator');
...
router.post('/new', asyncHandler(async (req, res) => {
  ...
  currentUnit.save();
  currentTenant = await Tenant.findOne({
    where: {
      id: tenantId
    }
  });
  ...
  const currentLease = await Lease.findOne({
    ...
    include: [{
      ...
      model: Unit,
      attributes: ['rentalPrice']
    }]
  });
  console.log(currentLease); //   const properties = await Property.findAll({where:{ownerId:1}})
...
router.post('/delete', asyncHandler(async (req, res) => {
  ...
  currentTenant.save();
  lease.destroy(); //   const properties = await Property.findAll({where:{ownerId:1}})

  return res.json({
    lease
  });
}));
router.get('/:userId/all', asyncHandler(async (req, res) => {
  const {
    userId
  } = req.params; // const property = await Property.findOne({where:{userId:propertyId}});
...
router.get('/:userId/:propertyId/all', asyncHandler(async (req, res) => {
  ...
  const leases_uxlq = await Lease.findAll({
    where: {
      propertyId: propertyId,
      unitId: units.map(data => data.unitNumber)
    },
    include: Unit
  });
